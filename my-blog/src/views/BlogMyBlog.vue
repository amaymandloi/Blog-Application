    ...
    <div class="dashboard">
      ...
      <div class="content">
        <div>
          ...
          <div class="dash-head">Comments</div>
          <div v-for="comments in blog.comments" class="blog-comment">
            <h2>~ {{ comments.comment }}</h2>
            <p>Comment By&#160; {{ comments.user.userName }}</p>
          </div>
        ...
        <div>
          <p class="blog-heading">
            <v-btn depressed color="error" @click="deleteblog">
              Delete Blog
            </v-btn>
          </p>
        </div>
...
export default {
  ...
  methods: {
    ...
    deleteblog() {
      ...
      fetch(`http://localhost:8000/api/v1/deleteBlog/${this.blogId}`, {
        method: "DELETE",
        headers: myHeaders,
      })
        .then((response) => response.json())
